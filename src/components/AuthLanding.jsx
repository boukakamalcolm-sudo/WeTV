...
export default function AuthLanding({ onContinueLocal }) {
  ...
  const submit = async (event) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        const result = await creerCompteAvecEmail(email, password);
        if (result?.requiresConfirmation) {
          setMessage('Compte créé. Vérifie ton adresse e-mail pour confirmer ton compte.');
        }
      } else if (mode === 'login') {
        await seConnecterAvecEmail(email, password);
      } else if (mode === 'reset') {
        await resetMotDePasse(email);
        setMessage('Un lien de réinitialisation vient d’être envoyé si cette adresse existe.');
      }
    } catch (e) {
      ...
      {mode === 'landing' ? (
        <div className="auth-content">
          ...
          <div className="auth-actions">
            ...
            <button className="auth-secondary" type="button" onClick={() => setMode('login')}>
              Se connecter avec un e-mail
            </button>
          </div>

          <p className="auth-signup">Pas encore de compte ? <button type="button" onClick={() => setMode('signup')}>Créer un compte</button></p>
          <button type="button" className="auth-local" onClick={onContinueLocal}>Continuer sans compte</button>
        </div>
      ) : (
        <div className="auth-form-shell">
          <button className="auth-back" type="button" onClick={() => { setMode('landing'); setError(''); setMessage(''); }}>← Retour</button>
          ...
          {mode === 'login' && <button className="auth-text-link" type="button" onClick={() => setMode('reset')}>Mot de passe oublié ?</button>}
          {mode === 'login' && <p className="auth-signup">Pas encore de compte ? <button type="button" onClick={() => setMode('signup')}>Créer un compte</button></p>}
          {mode === 'signup' && <p className="auth-signup">Déjà un compte ? <button type="button" onClick={() => setMode('login')}>Se connecter</button></p>}
        </div>
      )}
    </section>
  );
