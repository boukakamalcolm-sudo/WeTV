...
export default function WatchFlowHome() {
  ...
  async function addTitle(t){
    await ajouterItem(t);
    notifier(`${t.title} ajouté à ta bibliothèque`);
  ...
  return <div className="watchflow-home">
    ...
    {adding && <section className="quick-add"><label htmlFor="quick-add-input">Ajouter une série ou un film</label><div className="home-search"><span>⌕</span><input id="quick-add-input" autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Commence à taper un titre…" autoComplete="off" /></div>{searching&&<p className="subtitle">Recherche…</p>}{!searching&&results.length>0&&<div className="quick-add-results">{results.slice(0,6).map(t=><button key={`${t.mediaType}-${t.tmdbId}`} type="button" onClick={()=>addTitle(t)}><img src={poster(t.posterPath,'w185')} alt=""/><span><strong>{t.title}</strong><small>{t.mediaType==='tv'?'Série':'Film'}{t.year?` · ${t.year}`:''}</small></span><b>＋</b></button>)}</div>}{!searching&&query.trim().length>=2&&!results.length&&<p className="subtitle">Aucun titre trouvé.</p>}</section>}

    <section><div className="section-title-row compact"><div><p className="eyebrow">EN COURS</p><h2>Continuer à regarder</h2></div>{watching.length>0&&<a className="link-btn" href="#/bibliotheque">Tout voir →</a>}</div>{loading?<div className="watchflow-empty">Chargement de ta bibliothèque…</div>:watching.length?<div className="watchflow-cards">{watching.slice(0,6).map((show,i)=><motion.article key={show.localId} className={`watchflow-card ${i===0?'featured':''}`} whileTap={{scale:.99}}><div className="watchflow-art" style={{backgroundImage:`url(${show.art||''})`}}/><div className="watchflow-overlay"/><div className="watchflow-copy"><span>{show.mediaType==='movie'?'FILM':'SÉRIE'}</span><h3>{show.title}</h3><p>{show.mediaType==='movie'?'À reprendre':`Saison ${show.prochaine} • Épisode ${show.prochain}`}</p><div className="watchflow-progress"><i style={{width:`${show.progress}%`}}/></div></div><button className="watchflow-play" onClick={()=>markAsSeen(show)} aria-label="Marquer comme vu">✓</button></motion.article>)}</div>:<div className="watchflow-empty"><strong>Ta prochaine soirée commence ici.</strong><p>Ajoute une série ou un film pour voir ton suivi apparaître ici.</p><button className="watchflow-add" type="button" onClick={()=>setAdding(true)}>Chercher un titre</button></div>}</section>
